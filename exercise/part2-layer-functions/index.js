...
let campusGeojsonUrl = 'https://opendata.arcgis.com/api/v3/datasets/8ad76bc179cf44bd9b1c23d6f66f57d1_0/downloads/data?format=geojson&spatialRefId=4326';

function getStyle(school) {
  return { color: campusColors[school.properties.NAME] };
}
...
function displayData(data) {
  ...
  data.features.forEach(school => {
    schools.push(L.geoJSON(school, {
      style: getStyle(school),
    }).bindTooltip(school.properties.NAME));
  });

  L.layerGroup(schools).addTo(map);
}

fetch(campusGeojsonUrl)
